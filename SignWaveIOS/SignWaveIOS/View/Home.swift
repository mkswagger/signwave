...
struct Home: View {
    ...
    var attributedTitle: AttributedString{
        var attString = AttributedString(stringLiteral: "Sign Language - where ")
        if let range = attString.range(of: "Sign Language -"){
    ...
    var attributedSubTitle: AttributedString{
        var attString = AttributedString(stringLiteral: "everyone has a voice!")
        if let range = attString.range(of: "voice!"){
            attString[range].foregroundColor = .white
        }
        return attString
    }
...
struct DetailView: View{
    ...
    var body: some View{
        VStack{
            ...
            HStack(spacing: 0){
                ForEach(["Details","Info"],id: \.self){order in
                    Text(order)
